...
the interpretation of Granger causality is as a means of uncovering a plausible
...
\hl{TODO: From the gene network papers, it looks like these gene
  networks may actually have strongly-causal structure.  i.e. we can
  highlight the fact that our deeper understanding of a special
  problem case may in fact have significant applicability in these
  application areas.}

Other citations: support recovery consistency for the weighted lasso
in univariate AR(p) models:
...
\cite{autoregressive_process_modelling_via_the_lasso_procedure}.
Variations on the LASSO have been applied in a number of different
contexts \cite{DBLP:journals/corr/HallacPBL17} \cite{haufe2008sparse}
\cite{bolstad2011causal} \cite{he2013stationary}
\cite{grouped_graphical_granger_modelling_for_gene_expression_regulatory_networks_discovery}.
One of the key improvements to the original LASSO algorithm is the
adaptive (i.e. weighted) adaLASSO \cite{adaptive_lasso_zou2006}, for
which oracle results are established.  The crux of the matter for time
series analysis is to craft a good weighting scheme, see e.g.
\cite{discovering_graphical_Granger_causality_using_the_truncating_lasso_penalty}.
Success of the LASSO has lead to the famous ``bet on sparsity''
principle -- that is, for large problems and finite sources of data,
you might as well bet that a sparse model is correct, because if it
isn't, there is no hope.
...
and show that $VAR$ models having this structure satisfy naturaal
intuitive notions of ``information flow'' through the graph.
Moreover, we show that such graphs are perfectly (asymptotically)
recoverable with only \textit{pairwise} Granger-causality tests.  Our
finite sample results are based on simulations, where we show
promising results for these graphs topologies where our algorithm
...
\todo{What graphs in practice may be strongly causal -- see the gene network papers.}
...
\todo{Simulations with very small T (i.e. T = 20)}
...
\begin{align}
  ...
  H_1:&\ \widehat{x}_i^{(p_{ij})}(t) = \sum_{\tau = 1}^{p_{ij}} b_{ii}(\tau)x_i(t - \tau) + \sum_{\tau = 1}^{p_{ij}}b_{ij}(\tau)x_j(t - \tau).
\end{align}

Under the null hypothesis $H_0$ (i.e. that $x_j \npwgc x_i$) we can
form the asymptotically $\chi^2(p_{ij})$ distributed test statistic
\todo{Is this actually $\chi^2$ when $p_{ij}$ is chosen from the data?  This is adaptive!}

\begin{equation}
  \label{eqn:gc_statistics}
  F_{ij}(p_i, p_{ij}) = \frac{T}{p_{ij}}\Big(\frac{\xi_i(p_i)}{\xi_{ij}(p_{ij})} - 1\Big),
\end{equation}
...
and similarly for $\xi_{ij}(p_{ij})$.  Under moment conditions $x(t)$
(\hl{TODO: What assumptions? -- this must also depend on choice of
  $p_i, p_j$}) we have the convergence in probability:

\begin{equation}
  F_{ij}(p_i, p_{ij}) \overset{\P}{\rightarrow}
  \left\{
    \begin{array}{ll}
      0;\ x_j \npwgc x_i\\
      \infty;\ x_j \pwgc x_i
...
\ref{alg:finite_pwgc} asymptotically implements Algorithm
...
We set $p_{\text{max}} = 15$.  Results and representitive graphs are
collected in Figure \ref{fig:simulation_results}.

We compare our results against the LASSO as a baseline, estimating the
filter $\B(z)$ that minimizes $\xi_{\text{LASSO}}$ in the following:
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \includegraphics[width=\linewidth]{dag_increasing_q_simulation.pdf}
  \end{subfigure}
\end{figure}
...
still markedly superior in the case of a more general DAG.
...
demonstrating of this scalability, where we are able to estimate
graphs having over $1500$ nodes (over $2.25 \times 10 ^6$ possible
edges) using only $T = 500$ data points, granted, an SCG on this many
nodes is extremely sparse.  Application to denser graphs is explored
in Figure \ref{fig:simulation_results_dense}, where $n$ and $T$ are
held fixed while increasing the edge probability $q$, thereby creating
denser graphs.  For larger values of $q$ in this plot there is a
comparable number of edges as there are data samples $T$.
