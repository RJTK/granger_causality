...
\hl{TODO: Citation for this scheme}
In performing pairwise estimates $x_j \pwgc x_i$ we follow the simple
scheme of estimating the following two linear models via ordinary
least squares:

\begin{align}
  H_0:&\ \widehat{x}_i^{(i)}(t) = \sum_{\tau = 1}^{p_i} b_{ii}(\tau)x_i(t - \tau),\\
  H_1:&\ \widehat{x}_i^{(i, j)}(t) = \sum_{\tau = 1}^{p_i} b_{ii}(\tau)x_i(t - \tau) + \sum_{\tau = 1}^{p_j}b_{ij}(\tau)x_j(t - \tau).
\end{align}

Under the null hypothesis $H_0$ we can form the asymptotically
$\chi^2(p_j)$ distributed test statistic

\begin{equation}
  F_{ij} = \frac{T}{p_j}\Big(\frac{\xi_i}{\xi_{ij}} - 1\Big),
\end{equation}

where $\xi_i$ is the sample mean square of the residuals $x_i(t) - \widehat{x}^{(i)}_i(t)$, and similarly for $\xi_{ij}$.  Given additional assumptions on $x(t)$ (\hl{TODO: What assumptions?}) we have the convergence (\hl{a.s. or in P?})
...
Therefore, we can make the statement that Algorithm \ref{alg:pwgr_heuristic} asymptotically implements Algorithm \ref{alg:pwgr}.  \hl{TODO: Modify Algorithm 2 to make this true.}

It is known (\hl{citation}) that $F_{ij}$ is sensitive to the choice of $p_i$ and $p_j$.
...
\begin{algorithm}
  ...
  \BlankLine
  \caption{Graph Recovery Heuristic $\mathsf{pw\_scg}(F, \delta, b, R)$}
  \label{alg:pwgr_heuristic}
  % \TitleOfAlgo{Pairwise Graph Recovery}
  \Input{Estimates of pairwise Granger-causaltiy statistics $F_{ij} = \ln \frac{\xi_{ij}}{\xi_i}$\\
    Sparsity parameters ($\delta, b, R$)}
  ...
  $W_\delta \leftarrow \{(i, j)\ |\ F_{ji} \ge \delta, F_{ji} > F_{ij}\}$  \texttt{\# candidate edges}\\
  $\mathcal{I}_0 \leftarrow \big(\sum_{j\in S: (j, i) \in W_\delta} F_{ij}, \mathsf{\ for\ }i \in S \big)$ \texttt{\# total incident strength for each node}\\
  $P_0 \leftarrow \mathsf{arg\_select\_min\_N}(\mathcal{I}_0, N=R)$  \texttt{\# select} $R$ \texttt{root nodes}\\
  \BlankLine
  ...
  \While{$S \ne \emptyset$}{
    ...
    $P_k \leftarrow \mathsf{arg\_select\_min\_N}(\mathcal{I}_k, N=b)$  \texttt{\# select} $b$ \texttt{branching nodes}\\
    \;
    \texttt{\# add strongest} $P_{k - r} \rightarrow P_k$ \texttt{edges and maintain strong causality}\\
    \For{$r = 1, \ldots, k$} 
    {
      \For{$(i, j) \in \mathsf{sort}\Big(\{(i, j) \in P_{k - r} \times P_k\ |\ (i, j) \in W_\delta\} \mathsf{\ by\ descending\ } F_{ji}\Big)$}{
        \If{no $\gcgpath{u}{j}$ path in $E$, for any $u \in \anc{i}$}{
...
We have implemented simple empirical experiments in Python
\cite{scipy}, in particular we leverage the LASSO implementation from
\texttt{sklearn} \cite{sklearn} and the random graph generators from
\texttt{networkx} \cite{networkx}.  We run the same experiment on
three separate graph topologies having $n = 75$ nodes: a tree (which
is strongly causal), a random directed acyclic graph, and an Erdos
Renyi random graph with edge inclusion probability of $2 / n$.
...
uniformly at random in a disc of radius $3 / 4$ (which guarantees
stability for acyclic graphs).  The resulting system is driven by
i.i.d. Gaussian random vectors, each component having random variance
$\sigma_i^2 = 1/2 + r_i$ where $r_i \sim \text{exp}(1/2)$.  Results
are obtained for $50$ random iterations per each of $6$ dataset
lengths $T \in \{100, 500, 1000, 2500, 5000, 10000\}$.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.30\textwidth}
    \caption{Random Tree}
    \includegraphics[width=\linewidth]{example_tree.pdf}
  \end{subfigure}
  \begin{subfigure}[b]{0.30\textwidth}
    \caption{Random DAG}
  ...
  \begin{subfigure}[b]{0.30\textwidth}
    ...
    \includegraphics[width=\linewidth]{example_gnp.pdf}
  \end{subfigure}

  \begin{subfigure}[b]{0.30\textwidth}
    \includegraphics[width=\linewidth]{random_tree_simulation.pdf}
  \end{subfigure}
  \begin{subfigure}[b]{0.30\textwidth}
    \includegraphics[width=\linewidth]{random_dag_simulation.pdf}
  \end{subfigure}
  \begin{subfigure}[b]{0.30\textwidth}
    % \includegraphics[width=\linewidth]{random_gnp_simulation.pdf}
    \missingfigure{}
  \end{subfigure}
\end{figure}
