...
\abstract{We study Granger-causality in the context of wide-sense
  ...
  to LASSO algorithms) when structural assumptions are met.}

\keywords{Granger-causality, causality graph, time series, vector autoregression,
  LASSO, gene regulatory networks, network learning}
...
LASSO \cite{wainwright2009sharp}.%   Success of the LASSO has lead to
...
based on simulations, where we show promising results for these graphs
...
in the Wold decomposition, generalizing in some sense the notion of
``feedback-free'' processes studied by \cite{caines1975feedback} in
close connection with Granger-causality.  We establish sufficient
conditions (sections \ref{sec:strongly_causal_graphs},
...
(sec \ref{sec:pairwise_algorithm}).  Secondly, we propose in section
\ref{sec:structure_learning} a graph search heuristic which implements
our theoretical results to finite data samples, specifying and
summarizing appropriate methods for hypothesis testing (section
\ref{sec:pairwise_hypothesis_testing}), model order selection (section
\ref{sec:model_order_selection}), computationally efficient estimation
(section \ref{sec:efficient_model_estimation}), and error rate
controls (section \ref{sec:error_rate_control}).  Our heuristics are
compared against the LASSO algorithm in Section
...
\ref{sec:application} we develop an example application.  And provide
concluding remarks on further open problems in our concluding section
\ref{sec:conclusion}.  \todo{do an application}
...
where $c(t)$ is a purely deterministic sequence\footnote{the purely
  ...
  \pi]$}, $v(t)$ is an uncorrelated sequence and $A(0) = I$.  We
...
\begin{definition}[Parents, Grandparents, Ancestors]
  ...
  The set of all $i$'s parents will be denoted $\pa{i}$.
  ...
  for a node to be it's own parent.  However, unless $\gcg$ is a DAG,
  a node can be it's own ancestor.  We will ocassionally need to
  explicitly exclude $i$ from $\anc{i}$, in which case we will write
  $\anc{i}\setminus \{i\}$.
\end{definition}
...
proposition is analogous to the definition of feedback free processes
...
$\A_{ij}(z) = 0$, however the implication
$j \in \anc{i} \implies \A_{ij}(z)$ need not hold.
...
\begin{remark}
  ...
  be persistent since these naturally result in equivalent an
...
\begin{example}
  process $x(t)$ generated by the $\VAR(1)$ model\footnote{Recall that
    ...
    case.}  having $\B(z) = Bz^{-1}$ where $B$, we will examine
  ...
  \begin{equation*}
    \Dg(\lambda)^r \lambda = Vz
  \end{equation*}

  had a solution in $z$ for every $r \ge N$, where $V$ is an
  $n \times n - 1$ full-rank matrix spanning the nullspace of $\beta$.
  That is, iterates of $\Dg(\lambda)$ applied to $\lambda$ would need
  to remain inside $\mathcal{N}(\beta)$.  This would imply that

  \begin{equation*}
    VV^\dagger \lambda^{r + 1} = \lambda^{r + 1},
  \end{equation*}

  i.e. that $\lambda^{r + 1}$ is an eigenvector of $VV^\dagger$ for an
  infinite number of integers $r$.  However, since there can only be a
  finite number of (unit length) eigenvectors, this cannot be the case
  unless every eigenvalue $(\lambda_1, \ldots, \lambda_n)$ were equal.

  \todo{This may remain true for any matrix with at least 2 distinct eigenvalues}
  We can conclude by stating that a $\VAR(1)$ system corresponding to
  matrix $B$ is guaranteed to be persistent if $B$ is diagonalizable
  and has more than $1$ distinct eigenvalue.
\end{example}
...
\begin{proof}
  ...
  Evidently, this is $0$ for each $F$ if and only if
  ...
  since in general $\tau_0(\A_{jk}) < \infty$ and for a persistent
  system $\tau_\infty(\A_{ik}) = \infty$.
\end{proof}
...
If disturbances are Gaussian, we will have the convergence

\begin{equation}
  F_{ij}(p) \overset{\P}{\rightarrow}
  \left\{
    \begin{array}{ll}
      0;\ x_j \npwgc x_i\\
      \infty;\ x_j \pwgc x_i
...
$p_j$ (see e.g. \cite{lutkepohl2005new}), but we will focus in
particular on the Bayesian Information Criteria (BIC).  The BIC is
substantially more conservative than the popular alternative Akaiake
Information Criteria (the BIC is also asymptotically consistent), and
since we are searching for \textit{sparse graphs}, we therefore prefer
the BIC, where we seek to \textit{minimize} over $p$:
...
between $0$ and some prescribed $p_\text{max}$.  In practice it is
sufficient to pick $p_\text{max}$ ad-hoc or via some simple heuristic
e.g. plotting the sequence $BIC(p)$ over $p$, though it is not
technically possible to guarantee that the optimal $p$ is chosen
(since there can in general be arbitrarily long lags from one variable
to another).
...
estimates $\xi_i(p_i)$ and $\xi_{ij}(p_{ij})$.  This requires fitting
a total of $n^2p_{\text{max}}$ autoregressive models, where the most
...
autocorrelation method which deploys the following autocovariance
estimates in the Yule-Walker equations:\footnote{The particular
  indexing and normalization given in equation
  \ref{eqn:covariance_estimate} is critical to ensure $\widehat{R}$ is
  positive semidefinite.  The estimate can be viewed as calculating
  the covariance sequence of a signal multiplied by a rectangular
  window.}
...
asymptotically $\chi^2(p_{ij})$ distributed, and denote by $G$ the
cumulative distribution function thereof.  We will define the matrix
...
$\alpha = 0.05$).  Applying the extremely simple Bonferroni method is
...
Producing graph estimates which are not strongly causal can be
achieved out by performing sequential estimates
$\widehat{x}_1(t), \widehat{x}_2(t), \ldots$ by estimating a strongly
causal graph with the residuals of the previous model as input, and
then refitting on the estimated sparsity pattern.  However we reserve
any analysis of this scheme for future work.
...
collected in Figure \ref{fig:simulation_results}.

We compare our results against the LASSO.  Motivated by scaling, we
split the squared error term into separate terms, one for each group of
incident edges on a node, and estimate the collection of $n$ incident
filters $\big\{\B_{ij}(z)\big\}_{j = 1}^n$ that minimizes
...
where we are choosing $\lambda$, the regularization parameter, via the BIC.
...
\begin{figure}
  ...
  {\scriptsize Comparison of PWGC and LASSO for $\VAR(p)$ model
    estimation.  The MCC is measured on the left axis in blue with
    circle markers, and the relative log mean-squared prediction error
    $\frac{\ln\ \tr \widehat{\Sigma}_v}{\ln\ \tr \Sigma_v}$ on the
    right axis in red with triangle markers.  We use $q$ to denote the
    edge probability in a random Erdos-Reny graph (modified to form a
    DAG).  Solid lines indicate ad-hoc regression estimates for
    visualization purposes only.

    Matthew's Correlation Coefficient (MCC)
    \cite{matthews1975comparison} synthesizes the confusion matrix
    into a single score appropriate for unbalanced labels and is
    calibrated to fall into the range $[-1, 1]$ with $1$ being perfect
    performance, $0$ being the performance of random guessing, and
    $-1$ being perfectly opposed.
  }

  \begin{subfigure}[b]{0.45\textwidth}
    \caption{PWGC vs adaLASSO (SCG)}
    \label{fig:simulation_results_comparison1}
    \includegraphics[width=\linewidth]{new_lasso_comparison_scg_simulation.pdf}
    % \missingfigure{}
  \end{subfigure}
  \begin{subfigure}[b]{0.45\textwidth}
    \caption{PWGC vs adaLASSO (DAG, $q = \frac{2}{n}$)}
    \label{fig:simulation_results_comparison2}
    \includegraphics[width=\linewidth]{new_lasso_comparison_dag_simulation.pdf}
    % \missingfigure{}
  \end{subfigure}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \includegraphics[width=\linewidth]{increasing_n_simulation.pdf}
  \end{subfigure}
  \begin{subfigure}[b]{0.45\textwidth}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \includegraphics[width=\linewidth]{new_mcc_comparison000.pdf}
  \end{subfigure}
\end{figure}
...
degradation in performance as the number of DAG edges increases while
...
graphs having over $1500$ nodes (over $2.25 \times 10 ^6$ possible
edges) using only $T = 500$ data points, granted, an SCG on this many
nodes is extremely sparse.  However, it is certainly possible to
improve the scalability of the LASSO as applied to time series data by
taking advantage of the block-Toeplitz covariance structure.
...
when the strong-causality assumption is not met, we have provided some
empirical evidence to suggest that our pairwise testing algorithm PWGC
can still outperform the LASSO and adaLASSO, both of which are
commonly employed in applications.  Application to more general graphs
may also be within reach via an iterative graph building process,
i.e. estimating a sequence of SCGs via PWGC applied to the residuals
...
\ref{sec:pairwise_hypothesis_testing}) therefore is likely to enable
