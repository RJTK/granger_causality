...
\begin{theorem}[Pairwise Recovery]
  ...
  \begin{algorithm}
    ...
    \BlankLine
    $W_0 \leftarrow \{(i, j)\ |\ i \pwgc j, j \npwgc i\}$  \texttt{\# potential edges}\\
    \While{$S_k \ne \emptyset$}{
      $P_k \leftarrow \{i \in S_{k - 1}\ |\ \forall s \in S_{k - 1}\ (s, i) \not\in W_k\}$  \texttt{\# parents of current layer}\\
      % $P_k \leftarrow P_{k - 1} \cup P_k$ \texttt{\# Expand parent set}\\
      $S_{k + 1} \leftarrow S_k \setminus P_k$ \texttt{\# remove from } $S$\\

      \;
      $C_k \leftarrow \{j \in S_{k + 1}\ |\ \forall s \in S_{k + 1}\ (s, j) \not\in W_k\}$   \texttt{\# potential children of }$P_k$\\

      \For{$r = 0, \ldots, k$}
      {
        $D_{kr} \leftarrow \{(i, j) \in P_{k - r} \times C_k\ |\ (i, j) \in W_k,\ \text{no } i \rightarrow j \text{ path in } E \}$\\%  \texttt{\# edges incident on }$C_k$\\
...
\begin{example}
  ...
  That we need to proceed backwards through $P_{k - r}$ can be seen from the example of figure \ref{fig:example_fig3}, where if $D_k = \{(i, j) \in \Big(\bigcup_{r = 0}^k P_{k - r}\Big) \times C_k\ |\ i \pwgc j \}$ above we would infer the false positive edge $2 \rightarrow 3$.  Moreover, the same example shows that if simply $D_k = \{(i, j) \in P_k \times C_k\ |\ i \pwgc j \}$, then the edge $1 \rightarrow 3$ would be missed.
...
\begin{proof}
  ...
  First, suppose that $i \in \pa{j}$ and that there is no
  $i \rightarrow j$ path in $E$.  Then since $i \gc j$ we have
  $i \pwgc j$ by proposition \ref{prop:pwgc_anc} so at least one of
  ...
  is already an $i \rightarrow j$ path in $E$.  Using an induction
  hypothesis that $E$ contains edges if and only if they are in
  $\gcg$, then either $(i, j) \in E$ correctly, or there is more than
  one $i \rightarrow j$ path in $\gcg$, which is not possible since
  $\gcg$ is strongly causal.

  Finally, suppose that $i \not \in \pa{j}$.  If $i \not \pwgc j$ then
  we will correctly leave $(i, j)$ absent from $E$, so suppose that
  $i \pwgc j$.  Then we must have either $i \in \anc{j}$ or
  $\anc{i} \cap \anc{j} \ne \emptyset$; suppose first the former.
...
\begin{proof}
  ...
  there are nodes in $\gcg$ without parents, thence if
  ...
  $\mathcal{E} \subseteq W_0$.  This follows since by Proposition
