...
\begin{equation}
  \label{eqn:bic}
  \begin{aligned}
    BIC_{\text{univariate}}(p) &= -(T - p - 1) \ln \xi_i(p) - p\ln T,\\
    BIC_{\text{bivariate}}(p) &= -(T - p - 1) \ln \xi_{ij}(p) - 2p\ln T.\\
  \end{aligned}
\end{equation}

We carry this out via the forward search described in Algorithm
\ref{alg:maximize_bic}.  Since $BIC(p)$ need not be a convex function,
this scheme is not guaranteed to find the true maximum of the BIC, but
this approximation suffices for our purposes \hl{TODO: is this
  sufficient?  A full search is not so much harder.}
