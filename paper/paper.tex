...
$x(t)$ taking elements in $L_2$.  We suppose that $x(t)$ (with
$t \in \Z$) has zero mean, $\E x(t) = 0$, and the matrix valued
covariance sequence
$R(\tau) \overset{\Delta}{=} \E x(t)x(t - \tau)^\T$ is absolutely
summable and has a spectra $S(\omega)$:
...
\begin{proof}
  Equation \eqref{eqn:parent_expansion} was derived prior to the
  statement of the theorem.  We are left to demonstrate equation
  \eqref{eqn:ancestor_expansion}.
...
information'' in the underlying graph.  It may also be useful for
reasoning about what the conditions under which \textit{unobserved}
components of $x(t)$ may or may not interfere with inference in the
actually observed components.  Finally, motivated from a practical
standpoint to analyze causation in large systems, we will seek to
construct practical estimation procedures based purely on pairwise
causality tests which can result in reduced computational complexity.
...
\begin{proof}
  ...
  which follows from Proposition \ref{prop:ancestor_uncorrelated}.
  ...
  \While{$S \ne \emptyset$}{
    ...
    \For{$r = 1, \ldots, k$} 
    {
      \For{$(i, j) \in \mathsf{sort}\Big(\{(i, j) \in P_{k - r} \times P_k\ |\ (i, j) \in W_\delta\} \mathsf{\ by\ descending\ } F_{ji}\Big)$}{
        \If{\textit{no }$\gcgpath{i}{j}$\textit{ path in }$E$}{
...
\begin{algorithm}
  ...
  \Input{Wide sense stationary process $x(t)$\\
  ...
  \For{$m = 1, \ldots, M$}{
    ...
    $\epsilon^{(m)} \leftarrow \epsilon^{(m - 1)}(t) - \widehat{\B}^{(m)}(z)\epsilon^{(m - 1)}(t)$ \texttt{\# updated error process}\\
  }
  \BlankLine
