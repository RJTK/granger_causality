...
\ref{sec:application} we develop an example application.  Concluding
remarks on further open problems and extentions are provided in Section
...
and are broken into two groups: control and alcoholic.  The dataset
then consists of
$\mathcal{D} = \{\big(x^{(i)}(t)\big)_{t = 1}^T, y^{(i)} \}_{i = 1}^N$
where $T = 256$, $x^{(i)}(t) \in \R^{64}$,
$y^{(i)} \in \{\text{A}, \text{C}\}$, and \todo{fill in N} $N = ????$.

We have constructed a simple pipeline for classifying each each trial
as either ``A'' or ``C'' by first applying an iterative PWGC algorithm
(see Algorithm \ref{alg:iterated_pwgc}) directly to the EEG data
$x^{(i)}$ to obtain a Granger-causality graph $\widehat{G}^{(i)}$ then
simply feeding the vectorized adjacency matrix of $\widehat{G}^{(i)}$
through a Kernel PCA stage and then a Support Vector Machine with
...
Figures \ref{fig:ar_eeg_example} and \ref{fig:eeg_oos_eeror}
...
\begin{figure}
  ...
  {\scriptsize Figure \ref{fig:ar_eeg_example} provides an example of an
  autoregressive model fir via the Levinson-Durbin algorithm (choosing
  the system order via the BIC) on a single EEG channel.  Figure
  \ref{fig:eeg_oos_error} demonstrates the improvement of a single
  $\VAR$ model over independent $\mathsf{AR}$ models.
