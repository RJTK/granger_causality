...
\begin{theorem}[Pairwise Recovery]
  ...
  \begin{algorithm}
    ...
    \BlankLine
    $W_0 \leftarrow \{(i, j)\ |\ i \pwgc j, j \not\pwgc i\}$  \texttt{\# potential edges}\\
    \While{$S_k \ne \emptyset$}{
      $\widehat{P}_k \leftarrow \{i \in S_{k - 1}\ |\ \forall s \in S_{k - 1}\ (s, i) \not\in W_k\}$  \texttt{\# parents of current layer}\\
      ...
      \For{$r = 0, \ldots, k$}
      {
        $D_{kr} \leftarrow \{(i, j) \in \widehat{P}_{k - r} \times C_k\ |\ (i, j) \in W_k,\ \text{no } i \rightarrow j \text{ path in } E \}$\\%  \texttt{\# edges incident on }$C_k$\\
