...
\begin{remark}[Error Measurement]
  We we estimate the 1-step ahead prediction error by forming the variance matrix estimate
...
\begin{figure}
  \centering
  \caption{PWGC Compared Against AdaLASSO \cite{adaptive_lasso_zou2006}}
  ...
  {\scriptsize Comparison of PWGC and LASSO for $\VAR(p)$ model
    ...
    \ref{thm:scg_recovery}.  Figure
    \ref{fig:simulation_results_comparison2} provides results for
    systems which do not guarantee the assumptions of Theorem
    \ref{thm:scg_recovery}, though the graph has a similar level of
    sparsity.}
\end{figure}
...
\begin{figure}
  ...
  {\scriptsize Figure \ref{fig:simulation_results_scaling} depicts
    ...
    held fixed.  Remarkably, performance appears to degrade slowly in
    $n$ -- it is the graph topology (SCG or non-SCG) as well as the
    level of sparsity (measured by $q$) which is the main factor for
    support recovery performance.

    Figure \ref{fig:small_T_comparison} provides a support recovery
    comparison for very small values of $T$ typical of some
    applications.}
\end{figure}
