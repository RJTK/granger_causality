...
forests), DAGs where each node has at most one parent, and each of the
figures of this paper.  Fortunately, the strongly causal property is
  ...
  \begin{algorithm}
    ...
    \While{$S_k \ne \emptyset$}{
      ...
      \For{$r = 0, \ldots, k$}
      {
        ...
        $D_{kr} \leftarrow \{(i, j) \in P_{k - r} \times C_k\ |\ (i, j) \in W,\ \text{no } i \rightarrow j \text{ path in } E \}$\\%  \texttt{\# edges incident on }$C_k$\\
...
\begin{example}
  ...
  next iteration, the edges $(3, 4), (2, 4)$ are added, and the false
  edge $(1, 4)$ is excluded due to the path
  $1 \rightarrow 3 \rightarrow 4$ in E.  Finally, edges
  $(3, 5), (4, 6)$ are added, and the false
  $(1, 5), (1, 6), (3, 6), (2, 6)$ edges are similarly excluded due to
  the ordering of the inner loop.
