...
The interpretation of the following persistence condition is that
...
\begin{remark}
  ...
  system matrix is diagonalizable and has at least two distinct
...
\begin{example}
  ...
  model\footnote{Recall that any $\VAR(p)$ model with $p < \infty$ can be
    written as a $\VAR(1)$ model, so we lose little generality in
    considering this case.}  having $\B(z) = Bz^{-1}$, we will examine
...
\begin{remark}
  ...
  effects of confounding by removing bidirectional edges.
\end{remark}
...
requirement of persistence allows confounding to be eliminated by
removing bidirectional edges(by Proposition
\ref{prop:persistence_converse} and Corollary
\ref{cor:bidirectional_edge}).  Without persistence, each confounded
pair would give rise to $4$ possible pairwise topologies consistent
with $\gcg$, one for each type of pairwise edge (no edge,
