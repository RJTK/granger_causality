...
\hl{Need to refine this to exclude $k \rightarrow i \rightarrow j$ from 2.}
  ...
  \hl{Moreover, if $j \not\in \anc{i}$ then $\exists u \in \anc{i} \cap \anc{j}$ such that $i$ is not contained in at least one $u \rightarrow \cdots \rightarrow j$ path.  [Must still prove this.]}
\end{proposition}
\begin{proof}
  ...
  For the second part of the theorem we exclude possibility $(1)$, namely, $j \not \in \anc{i}$ and suppose by way of contradiction that for every $u \in \anc{i} \cap \anc{j}$ 
\end{proof}
