...
\end{proposition}
\begin{proof}
  We will show that for some $\psi \in \H_{t - 1}^{(j)}$ we have $\inner{\psi}{x_i(t) - \linE{x_i(t)}{\H_{t - 1}^{(i)}}} \ne 0$ and therefore that $H_t^{(i)} \not\perp\ \H_{t - 1}^{(j)}\ |\ \H_{t - 1}^{(i)}$, which by theorem (\ref{thm:granger_causality_equivalences}) and \hl{lemma [name this lemma]} is enough to establish that $j \pwgc i$.

    Firstly, we will establish a representation of $x_i(t)$ that involves $x_j(t)$.  Denote by $a_{r + 1} \rightarrow a_r \rightarrow \cdots \rightarrow a_1 \rightarrow a_0$ with $a_{r + 1} \defeq j$ and $a_0 \defeq i$ the unique $j \rightarrow i$ path in $\gcg$, we will expand the representation of equation (\ref{eqn:parent_expansion}) backwards along this path:

\begin{align*}
  x_i(t) &= v_i(t) + \B_{ii}(z) x_i(t) + \sum_{k \in \pa{i}}\B_{ik}(z) x_k(t)\\
         &= \underbrace{v_i(t) + \B_{ii}(z) x_i(t) + \sum_{\substack{k \in \pa{i} \\ k \ne a_1}}\B_{ik}(z) x_k(t)}_{\defeq \wtalpha{a_0}{a_1}} + \B_{a_0a_1}(z)x_{a_1}(t)\\
         &= \wtalpha{a_0}{a_1} + \B_{a_0a_1}(z)\big[\wtalpha{a_1}{a_2} + \B_{a_1a_2}(z)x_{a_2}(t) \big]\\
         &= \cdots\\
         &= \sum_{\ell = 0}^r \underbrace{\Big(\prod_{m = 0}^{\ell - 1} \B_{a_m a_{m + 1}}(z) \Big)}_{\defeq F_\ell(z)} \wtalpha{a_\ell}{a_{\ell + 1}} + \Big(\prod_{m = 0}^{r}\B_{a_m a_{m + 1}}(z)\Big)x_j(t)\\
         &= \sum_{\ell = 0}^r F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}} + F_r(z) x_j(t)
\end{align*}

where we have defined $\prod_{m = 0}^{-1} \bullet \defeq 1$ for notational convenience.

Then,

\begin{align*}
  \inner{\psi}{x_i(t) - \linE{x_i(t)}{&\H_{t - 1}^{(i)}}} = \inner{\psi}{F_r(z) x_j(t) - \linE{F_r(z) x_j(t)}{\H_{t - 1}^{(i)}}}\\ &+ \sum_{\ell = 0}^r \inner{\psi}{F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}} - \linE{F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}}}{\H_{t - 1}^{(i)}}}\\
  &\overset{(a)}{=} \inner{\psi}{F_r(z) x_j(t) - \linE{F_r(z) x_j(t)}{\H_{t - 1}^{(i)}}}\\ &-\sum_{\ell = 0}^r \inner{\psi}{\linE{F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}}}{\H_{t - 1}^{(i)}}}\\
  &= \inner{\psi}{F_r(z)x_j(t)} - \inner{\psi}{\linE{F_r(z)x_j(t) + \sum_{\ell = 0}^r F_{\ell}(z)\wtalpha{a_\ell}{a_{\ell + 1}}}{\H_{t - 1}^{(i)}}}
\end{align*}

where $(a)$ follows since $\forall \psi \in \H_{t - 1}^{(j)}\ \inner{\psi}{\wtalpha{a_\ell}{a_{\ell + 1}}} = 0$ which follows since for every $\ell = 1, \ldots, r$ we have $a_{\ell} \not\in \anc{j}, j \not\in \anc{a_{\ell}}$ and \hl{an application of lemmas [name these]}.  Continuing from the above we will choose the particular value $\psi = F_r(z)x_j(t)$, then

\begin{align*}
  &\inner{F_r(z)x_j(t)}{x_i(t) - \linE{x_i(t)}{\H_{t - 1}^{(i)}}} = 0\\
  &\iff \E|F_r(z)x_j(t)|^2 = \inner{F_r(z)x_j(t)}{\linE{F_r(z)x_j(t) + \sum_{\ell = 0}^r F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}}}{\H_{t - 1}^{(i)}}}\\
  &\overset{(b)}{\iff} F_r(z)x_j(t) = \linE{F_r(z)x_j(t) + \sum_{\ell = 0}^r F_\ell(z) \wtalpha{a_\ell}{a_{\ell + 1}}}{\H_{t - 1}^{(i)}},
\end{align*}

where $(b)$ follows by the condition for equality in the Cauchy-Schwarz inequality.  But since $F_r(z)x_j(t) \not \in \H_{t - 1}^{(i)}$, we must have $\inner{\psi}{x_i(t) - \linE{x_i(t)}{\H_{t - 1}^{(i)}}} \ne 0$ and therefore $j \pwgc i$.
